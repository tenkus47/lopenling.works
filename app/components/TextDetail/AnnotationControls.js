...
export type Props = {
  ...
  addLineBreak: () => void,
  addQuestion: () => void,
  saveQuestion: (question: Annotation, title: string, content: string) => void,
  list: List | null,
...
class AnnotationControls extends React.Component<Props> {
  ...
  updatePosition() {
    ...
    if (anchorPoint === anchorPoints.bottom) {
      ...
      if (
        measurements.top + measurements.height + bottomGap - controlsTop <
        height
      ) {
        controlsTop =
  ...
  getMeasurements(): {
    ...
    const lastSelectedElementId =
      this.props.selectedElementIds[this.props.selectedElementIds.length - 1];
  ...
  render() {
    ...
    if (props.annotationsData) {
      props.annotationsData.map((annotationData) => {
        ...
        if (annotationData.isTemporary) {
          ...
          annotationData.annotation.uniqueId == props.activeAnnotation.uniqueId
        ) {
          isActive = true;
        }
      ...
      if (this.props.availableAnnotations) {
        pageBreaks = this.props.availableAnnotations.filter(
          (annotation: Annotation) =>
            ...
            annotation.creatorWitness.id === this.props.selectedWitness.id
        );
    ...
    if (
      ...
      tempQuestions = props.temporaryQuestions.map((question: Annotation) => {
    ...
    if (props.questions && props.questions.length > 0) {
      ...
      for (let i = 0; i < props.questions.length; i++) {
        ...
        let isValid = !tempQuestionIds.hasOwnProperty(question.uniqueId);
        if (isValid) {
    ...
    if (questionViews.length > 0) {
      questionHeading = <h3 className={styles.sectionHeading}>Questions</h3>;
    }
    ...
    let allowQuestion =
      props.questions.length === 0 && props.temporaryQuestions.length === 0;
    ...
    return (
      <div
        ...
        ref={(controls: HTMLDivElement | null) => (this.controls = controls)}
      >
        {showHeader && (
          <AnnotationControlsHeader
            ...
            addNote={isLoggedIn && !tempNotes ? () => props.addNote() : null}
