...
import {
  ...
  IMAGE_URL_SUFFIX,
  IMAGE_START_PRE_KEY,
  IMAGE_START_NUMBER_KEY,
  IMAGE_START_SUFFIX_KEY,
} from "app_constants";
...
export default class SplitTextComponent extends React.PureComponent<Props> {
  ...
  getControlsMeasurements(props: Props): {
    ...
    if (props.activeAnnotation) {
      ...
      [startPos] =
        props.splitText.annotatedText.getPositionOfAnnotation(activeAnnotation);
      if (startPos === null) {
    ...
    if (
      ...
      for (let i = 0; i < selectedAnnotatedSegments.length; i++) {
        ...
        if (firstSelectedSegment === null && segment instanceof TextSegment) {
      ...
      if (props.activeAnnotation.isDeletion) {
        ...
        const [start] = props.splitText.annotatedText.getPositionOfAnnotation(
          props.activeAnnotation
        );
        if (start) {
  ...
  lineBreaksChanges(oldProps: Props, newProps: Props) {
    ...
    if (
      ...
      !newProps.activeAnnotations.hasOwnProperty(oldActiveAnnotation.uniqueId)
    ) {
      hasChanged = true;
    }
    ...
    if (
      ...
      !oldProps.activeAnnotations.hasOwnProperty(newActiveAnnotation.uniqueId)
    ) {
      hasChanged = true;
    }
  ...
  processProps(props: Props) {
    ...
    if (
      ...
      if (changedWitness) {
        ...
        let selectedRows = [];
        if (selectedRow !== null) {
          for (let i = selectedRow, len = splitRowTexts.length; i < len; i++) {
            selectedRows.push(i);
          }
          this.updateList(true, selectedRows);
        }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (con && this.props.searchResults) {
      if (this.SearchSyncId) {
        let selectedTextIndex = this.props.splitText.getTextIndexOfPosition(
          this.SearchSyncId
        );
        setTimeout(() => {
    ...
    if (this.selectedNodes && this.selectedNodes.length > 0) {
      ...
      setTimeout(() => {
        ...
        if (lastSegment instanceof TextSegment) {
          let lastElement = document.getElementById(idForSegment(lastSegment));
  ...
  getSelectedTextIndex(): number {
    ...
    if (this.props.activeAnnotation) {
      [startPos] = this.props.splitText.annotatedText.getPositionOfAnnotation(
        this.props.activeAnnotation
      );
  ...
  getBaseAnnotation(annotation: Annotation): Annotation {
    let [start] =
      this.props.splitText.annotatedText.getPositionOfAnnotation(annotation);
  ...
  getImageUrl(pageIndex: number): string {
    if (!this.props.selectedWitness || !this.props.selectedWitness.properties)
      return "";
    ...
    let url = IMAGE_URL_PREFIX + prefix + id + "." + suffix + IMAGE_URL_SUFFIX;
    return url;
  }
      ...
      >
        ...
        >
          <div className={styles.splitTextRowContent}>
            {props.showImages && (
              <div className={pechaImageClass} style={pechaStyles}>
                <img
                  ...
                  onLoad={(e) => {
                    if (e.target && component.imageWidth === null) {
                      ...
                      window.setTimeout(
                        component.updateList.bind(component),
                        0
                      );
                    }
                  }}
