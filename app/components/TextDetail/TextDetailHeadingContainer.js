...
const mapStateToProps = (state: AppState): {} => {
  ...
  if (selectedText) {
    ...
    if (selectedWitnessId) {
      ...
      exportingWitness = reducers.getExportingWitness(state, selectedWitnessId);
    } else {
  ...
  if (selectedText2) {
    ...
    if (selectedWitnessId2) {
      ...
      selectedWitness2 = reducers.getWorkingWitness2(state, selectedText2.id);
    }
  }
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  return {
    ...
    navigationButtonClicked: () => {
      dispatch(actions.changedTextListVisible(!stateProps.textListIsVisible));
    },
